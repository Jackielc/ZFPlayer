...
def read_file_to_json(file_name):
    ...
    res['consumptionTime'] = caculate_time_interval(res)
    return res
...
def send_log(client, project, logstore, log):
    topic = ''
    source = '识货App工程编译时间统计'
    extra = {
        'event_name': 'com.shsentry.iosPackageTime',
    }
    extra.update(log)
    extraString = json.dumps(extra)
    contents = [
        ('device_id', 'abcdefghijklmnopq'),
        ('extra', extraString)
    ]
    ...
    request = PutLogsRequest(project, logstore, topic,
                             source, logitemList, compress=False)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        cmd = '执行upload_event_to_sls.py'
        shell_result = os.system(cmd)

        log_path = sys.argv[1]
        client = get_log_client()
        log = read_file_to_json(log_path)
        send_log(client, project_name, logstore_name, log)
    else:
        #print('未获取到日志文件路径，退出')
        cmd = 'echo 未获取到日志文件路径，退出'
        shell_result = os.system(cmd)
